...
function InvoicesTab() {
  ...
  const { data: mandatesByClient = new Map<string, any>() } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const m of data ?? []) map.set(m.client_id, m.is_active);
      return map;
    },
  });
  ...
  const statusBadgeVariant = (status: string) => {
    ...
    if (status === "included") return "secondary";
  ...
  const statusColor = (status: string) => {
    ...
    if (status === "included") return "bg-blue-100 text-blue-800";
...
function HistoryTab() {
  ...
  const selected = remittances.find((r) => r.id === selectedRemittance);
      ...
      {selected && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Detalles de remesa</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 text-sm">
            <div>
              <div className="text-xs text-muted-foreground">Message ID</div>
              <div className="font-mono font-medium">{selected.message_id}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">Acreedor</div>
              <div className="font-medium">{selected.creditor_name}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">IBAN Acreedor</div>
              <div className="font-mono text-xs">{formatIban(selected.creditor_iban)}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">Fecha de cobro</div>
              <div>{selected.collection_date}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">Recibos</div>
              <div className="font-mono font-medium">{selected.transaction_count}</div>
            </div>
            <div>
              ...
              <DialogContent>
                ...
                <Select value={newStatus} onValueChange={setNewStatus}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="with_returns">Con devoluciones</SelectItem>
                  </SelectContent>
                </Select>
                <DialogFooter>
